...
layout (rgba8, binding = 1) readonly uniform image3D imgVoxels;
...
{
	ivec3 gridSize = imageSize(imgVoxels);
	return uint(((gridSize.x * gridSize.y * index.z) + (gridSize.x * index.y) + index.x));	
}

ivec3 expandIndex(uint index)
{
	ivec3 parsed;
	ivec3 gridSize = imageSize(imgVoxels);
	parsed.x = int(floor(index % gridSize.x));
	parsed.y = int(floor((index / gridSize.x) % gridSize.y));
	parsed.z = int(floor(index / (gridSize.x * gridSize.y)));
	return parsed;
}
...
uint getVoxelMaterial(ivec3 index, Chunk chunk)
{
	...
	vec4 unpacked = imageLoad(imgVoxels, expandIndex(smallIndex));
	switch (buff)
	{
